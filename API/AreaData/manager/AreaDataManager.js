  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (areas && areasCount && resultTotalDayViews && resultTotalWeekViews && resultTtotalMonthViews && resultTotalYearViews && resultTotalViews) {
        ...
        resolve({ data: areas, total: areasCount[0].count, totalDayViews: totalDayViews, totalWeekViews: totalWeekViews, totalMonthViews: totalMonthViews, totalYearViews: totalYearViews, totalViews: totalViews });
      } else {
        resolve({ data: [], total: 0, totalDayViews: 0, totalWeekViews: 0, totalMonthViews: 0, totalYearViews: 0, totalViews: 0 });
      }
    } catch (e) {
      Logger.error(__filename, e);
      reject("failed");
