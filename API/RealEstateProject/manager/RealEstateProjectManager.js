     ...
     try {
      ...
      let searchText = req.payload.searchText;
       ...
       if (realEstateProjects && realEstateProjectCount) {
         resolve({data: realEstateProjects, total: realEstateProjectCount[0].count});
       }else{
  ...
  async function uploadImage(req) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (image) {
          var result = await RealEstateProjectResourceAccess.updateById(id, {introduceImage: image});
          if(result){
            resolve(result);
          }
        } else {
  ...
  async function updateStatusNote(req) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let result = await RealEstateProjectResourceAccess.updateById(id, {statusNote: statusNote});
        if(result){
          resolve(result);
        }
        reject("failed");
  ...
  async function exportExcelFile(req) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let endDate = req.payload.endDate;
        let searchText = req.payload.searchText;
        let realEstateProjects = await RealEstateProjectResourceAccess.customSearch(filter, undefined, undefined, startDate, endDate, searchText, order);
