...
const AppDevicesResourceAccess = require("../AppDevices/resourceAccess/AppDevicesResourceAccess");
...
async function retrieveUserDetail(appUserId) {
    ...
    if (user && user.length > 0) {
        ...
        //get wallet data
        let devices = await AppDevicesResourceAccess.find({ appUserId: foundUser.appUserId });
        if (devices) {
    ...
    if (user && user.length > 0) {
        ...
        if (QRCodeImage) {
            await AppUsersResourceAccess.updateById(appUserId, {
                twoFACode: topSecret,
                twoFAQR: process.env.TWO_FA_SERVICE + `/User/get2FACode?appUserId=${appUserId}`
            })
            return QRCodeImage;
        }
    }
